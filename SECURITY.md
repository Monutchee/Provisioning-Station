...
Its bearer token protects the API but does not encrypt traffic. The default
listener accepts IPv4 LAN connections and automatically creates a private API
token in the Station data directory. Use a trusted TLS or mutually
authenticated proxy before allowing access outside a trusted provisioning
network. TFTP has no protocol-level authentication; isolate the provisioning
network and provide a board IP when possible so the agent can restrict the
TFTP client.
